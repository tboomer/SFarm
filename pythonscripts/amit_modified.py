...
# driver-detection/forums/t/19971/simple-solution-keras
...
def load_train(img_rows, img_cols):
    ...
    for j in range(10):
        ...
        path = os.path.join('C:/Users/tboom_000/Documents/Personal/Projects/Kaggle/StateFarm/input', 'train', 'c' + str(j), '*.jpg')
...
def load_test(img_rows, img_cols):
    ...
    path = os.path.join('C:/Users/tboom_000/Documents/Personal/Projects/Kaggle/StateFarm/input', 'test', '*.jpg')
    files = glob.glob(path)
    X_test = []
    X_test_id = []
    total = 0
...
def create_submission(predictions, test_id, loss):
    ...
    if not os.path.isdir('subm'):
        os.mkdir('subm')
    suffix = str(loss) + '_' + str(now.strftime("%Y-%m-%d-%H-%M"))
    sub_file = os.path.join('subm', 'submission_' + suffix + '.csv')
    result1.to_csv(sub_file, index=False)


def read_and_normalize_train_data(img_rows, img_cols):
    cache_path = os.path.join('C:/Users/tboom_000/Documents/Personal/Projects/Kaggle/StateFarm/cache', 'train_r_' + str(img_rows) + '_c_' + str(img_cols) + '.dat')
...
def read_and_normalize_test_data(img_rows, img_cols):
    cache_path = os.path.join('C:/Users/tboom_000/Documents/Personal/Projects/Kaggle/StateFarm/cache', 'test_r_' + str(img_rows) + '_c_' + str(img_cols) + '.dat')
    if not os.path.isfile(cache_path):
