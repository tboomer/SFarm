...
def get_im(path, img_rows, img_cols, color_type):
    try:
        ...
        # edges = cv2.Canny(resized, 200, 400) # If edge detection is required.
        return resized
...
def read_and_normalize_train_data(img_rows, img_cols, color_type):
    cache_path = os.path.join('.', 'cache', 'train_r_' + str(img_rows) + '_c_' + str(img_cols) + "_" + str(color_type) + '.dat')
...
def read_and_normalize_test_data(img_rows, img_cols, color_type):
    cache_path = os.path.join('.', 'cache', 'test_r_' + str(img_rows) + '_c_' + str(img_cols) + "_" + str(color_type) + '.dat')
    if not os.path.isfile(cache_path):
