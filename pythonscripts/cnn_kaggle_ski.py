# -*- coding: utf-8 -*-
# Starter script downloaded from Kaggle forum 4/28/2016 with kudos to ZFTurbo
...
import glob
#import cv2
from skimage.io import imread as skiread
...
def get_im_ski(path, img_rows, img_cols, color_type=1):
    # Load as grayscale
    if color_type == 1:
        img = skiread(path, as_grey=False)
    elif color_type == 3:
        img = skiread(path, as_grey=True)
    # Reduce size
    resized = skiresize(img, (img_cols, img_rows))
    return resized
...
def load_train(img_rows, img_cols, color_type=1):
    ...
    for j in range(10):  #10
        ...
        for fl in files:
            ...
            img = get_im_ski(fl, img_rows, img_cols, color_type)
            X_train.append(img)
            y_train.append(j)
...
def read_and_normalize_train_data(img_rows, img_cols, color_type=1):
    ...
    train_data = train_data.reshape(train_data.shape[0], 3, img_rows, img_cols) # 3 instead of color_type
...
def read_and_normalize_test_data(img_rows, img_cols, color_type=1):
    ...
    test_data = test_data.reshape(test_data.shape[0], 3, img_rows, img_cols) # 3 instead of color_type
...
#    random_state = 51
    ...
    model = create_model_v1(img_rows, img_cols, 3) #param 3 was color_type_global
